...
Data.length===0{
    ...
    const dataLoader = async () => {
        ...
        categories.forEach(category => {
            ...
            div.innerHTML = `
            <button onclick="handleLoadData('${category.category_id}')" class="btn inline-block rounded-lg ">${category.category}</button>
            `
    ...
    const handleLoadData = async (categoryId) => {
        ...
        if (data.dat.length ===0) {
            ...
            const div = document.createElement('div');
            div.innerHTML=`<h1>no content here</h1>`
      ...
      <div class="card-body">
       ...
       <div>
       <img class="rounded-full h-[40px] w-[40px]" src= ${info.authors[0].profile_picture}/>
       </div>
        <div>
